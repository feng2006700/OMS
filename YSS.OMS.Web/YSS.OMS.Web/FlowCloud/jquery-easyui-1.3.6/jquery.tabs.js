...
(function ($) {
    ...
    function setScrollers(container) {
        ...
        var leftTool = header.children('div.tabs-tool-left');
        var rightTool = header.children('div.tabs-tool-right');
        ...
        leftTool._outerHeight(tHeight);
        rightTool._outerHeight(tHeight);

        var tabsWidth = 0;
        $('ul.tabs li', header).each(function () {
            tabsWidth += $(this).outerWidth(true);
        });
        var cWidth = header.width() - leftTool._outerWidth() - rightTool._outerWidth();
        ...
        if (tabsWidth > cWidth) {
            ...
            if (opts.leftTools) {
                ...
                marginLeft += sLeft.outerWidth() + leftTool._outerWidth();
            ...
            if (opts.rightTools) {
                ...
                marginRight += rightTool._outerWidth();
            ...
            if (opts.leftTools) {
                ...
                /*wrap.css({
                    marginLeft: leftTool._outerWidth(),
                    marginRight: 0,
                    width: cWidth
                });*/
            }
    ...
    function addTools(container) {
        ...
        var header = $(container).children('div.tabs-header');
        if (opts.leftTools) {
            if (typeof opts.leftTools == 'string') {
                $(opts.leftTools).addClass('tabs-tool-left').appendTo(header);
                $(opts.leftTools).show();
            } else {
                header.children('div.tabs-tool-left').remove();
                var tools = $('<div class="tabs-tool-left"><table cellspacing="0" cellpadding="0" style="height:100%"><tr></tr></table></div>').appendTo(header);
                var tr = tools.find('tr');
                for (var i = 0; i < opts.leftTools.length; i++) {
                    var td = $('<td></td>').appendTo(tr);
                    var tool = $('<a href="javascript:void(0);"></a>').appendTo(td);
                    tool[0].onclick = eval(opts.leftTools[i].handler || function () { });
                    tool.linkbutton($.extend({}, opts.leftTools[i], {
                        plain: true
                    }));
                }
            }
        ...
        if (opts.rightTools) {
            if (typeof opts.rightTools == 'string') {
                $(opts.rightTools).addClass('tabs-tool-right').appendTo(header);
                $(opts.rightTools).show();
            } else {
                header.children('div.tabs-tool-right').remove();
                var tools = $('<div class="tabs-tool-right"><table cellspacing="0" cellpadding="0" style="height:100%"><tr></tr></table></div>').appendTo(header);
                var tr = tools.find('tr');
                for (var i = 0; i < opts.rightTools.length; i++) {
                    var td = $('<td></td>').appendTo(tr);
                    var tool = $('<a href="javascript:void(0);"></a>').appendTo(td);
                    tool[0].onclick = eval(opts.rightTools[i].handler || function () { });
                    tool.linkbutton($.extend({}, opts.rightTools[i], {
                        plain: true
                    }));
                }
            }
    ...
    function setSize(container) {
        ...
        if (opts.tabPosition == 'left' || opts.tabPosition == 'right') {
            ...
            var lrt = header.children('div.tabs-scroller-left,div.tabs-scroller-right,div.tabs-tool-left,div.tabs-tool-right');
    ...
    $.fn.tabs.parseOptions = function (target) {
        return $.extend({}, $.parser.parseOptions(target, [
			'width', 'height', 'tools', 'toolPosition', 'tabPosition', 'leftTools', 'rightTools',
